    ...
    if (difftime(curtime, req->sent) > 0.5)
    {
        ...
        if (req->times_sent >= 5)
        {
            ...
            while (pck != NULL) {
                ...
                /*payload*/
                memcpy(ict3_hdr->data, i_hdr0, sizeof(struct sr_ip_hdr) + 8);
                ...
                if (rtentry != NULL) {
					...
					entry = sr_arpcache_lookup(cache, next_hop_ip);
					if (entry != NULL)
					{
					/* Ethernet header */
					...
					{
						/* queue */
						struct sr_arpreq *arpreq = sr_arpcache_queuereq(cache, next_hop_ip, new_pck, new_len, rtentry->interface);
